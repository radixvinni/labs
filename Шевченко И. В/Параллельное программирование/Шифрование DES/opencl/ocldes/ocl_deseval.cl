...
unsigned long
deseval (
	__global unsigned long	*p,
	__global unsigned long	*c,
...
__kernel void keysearch(__global unsigned long* r, __global unsigned long* p, __global unsigned long* c, __global unsigned long* kk)
{	
	int i;
	unsigned long k[56];
	unsigned int n = get_global_id(0);
	for(i=0; i<56; i++) k[i]=kk[i];
	set_high_keys(k,n);
	//for (i=0; i<512; i++) {
	    unsigned long	res = deseval (p, c, k);
	    if(res!=0) 
			*r=res;
	//	increment_key (k);
	//}
