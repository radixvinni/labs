        ...
        Token nextToken()
        {
                ...
                switch(value[0])
                {
                        ...
                        default:
                                ...
                                throw Error("Нет такого символа", position);
                }
        }
};
//---------------------------------------------------------------------------
...
class AttributeParser
{
        ...
        struct TM //Token Magazine Node
        {
                ...
                int q;
                int s;
                int* r; //Внимание! Аттрибут r - наследуемый!
                //Используеся для передачи результата.
                //Хранит указатель на аттрибут, куда нужно записать номер операнда,
                //когда будет известен этот номер - ячейка, где хранится операнд.
                TM* n;
                TM():r(0) {}
        } *m, *f, *flast;
        //Выше описанная структура используется для хранения аттрибутов в магазине,
        //а также как выходная цепочка действий. любые попытки скопировать символы
        //действия в какой-то массив по мере поступления не принесут успеха.
        int n;
        private:
        Token t;
        void a(Token tt)//втолкнуть в магазин
        ...
        void p5(){v();} //Вытолкнуть, Держать
        void p6() //Заменить(=<E>;{Присвоить}), Сдвиг
        {
                int x=0;
        ...
        void p7() //Заменить((<лог.выр>){Переход по 0} <S> ИНАЧЕ {Прыжок} {Метка} <S> {Метка} КОНЕЦ_ЕСЛИ), Сдвиг
        {
                v();
                a(KEYWORD_ENDIF);
                a(FLABEL);
        ...
        void p8() //Заменить( ({Метка}<лог.выр.>)  {Условный переход по 0} <S> {Безусловный переход} {Метка} КОНЕЦ_ЦИКЛ), Сдвиг
        {
                v();
                a(KEYWORD_ENDLOOP);
                a(FLABEL);
        ...
        void p9() //Заменить(<T><E-список>), Держать
        {
                int* x = m->r;//туда положим результат. Передаём в список с аргументом.
                v();
                a(TELIST);
                m->r=x;
                a(TT);
        ...
        void p10() //Заменить(<T>{Сложить}<E-список>), Сдвиг
        {
                int p = m->p;
                int* rr = m->r;
                v();
        ...
        void p13() //Заменить(<F>{Умножить}<T-список>), Сдвиг
        {
                int p = m->p;
                int* rr = m->r;
                v();
        ...
        void p17() //Заменить(<F><лог.опер.><F>{Сравнение}), Держать
        {
                int* x = m->r;
                v();
                a(FCMP);
        ...
        void p18() //Вытолкнуть,Сдвиг
        {
                *(m->r)=1;//>
                v();
                t=r.nextToken();
        }
        void p19() //Вытолкнуть,Сдвиг
        {
                *(m->r)=-1;//<
                v();
                t=r.nextToken();
        ...
        public:
        /*struct tcode       //Это неудавшаяся реализация хранения символов действия
        {
                Token t;
                int p,q,s,r;
        } code[50];*/
...
class Translator //интерпретатор объектного кода
{
        public:
        int mem[500];
        int n;
